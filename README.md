...
Triggers are collected only for ordinary tables, because a trigger is written into its table's file and nothing else has one. So an `INSTEAD OF` trigger on a view is **not** dumped. It is skipped deliberately rather than collected and then dropped on the floor, but a schema that relies on one will restore without it.

**Identifiers containing a literal dot are not handled.** A qualified name is matched and keyed as `schema || '.' || name` — in the scope predicate, in the manifest entries, and in the internal grouping that attaches indexes, triggers and constraints to their table. That is unambiguous only while no identifier contains a dot itself, which Postgres permits when quoted (`CREATE SCHEMA "a.b"`). Schema `a.b` table `c` and schema `a` table `b.c` are indistinguishable to all of them, so such a schema can mis-scope or attach an object to the wrong table. Manifest validation rejects entries with more than one dot, which keeps the common typo out, but it is not a fix. Emitted DDL is unaffected — every identifier is quoted independently at emission.
